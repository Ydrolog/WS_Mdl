...
Pa_SFR_OBS_Src, Pa_SFR_OBS_Dst  =   PJ(Pa_Mdl, f"In/OBS/SFR/{MdlN_SFR_SRC}/{MdlN_SFR_SRC}.SFR.OBS6"), PJ(Pa_MdlN, f"GWF_1/MODELINPUT/{MdlN}.SFR.OBS6")
...
rule add_SFR_copy:
    ...
    output:
        Pa_SFR_Dst
    run:
        with open(Pa_SFR_Src, 'r') as f1:
            l_lines     = f1.readlines()
        with open(Pa_SFR_Dst, 'w') as f2:
            for i in l_lines:
                f2.write(i.replace(MdlN_SFR_SRC, MdlN)) # Replace the source model name with the current one.
