...
l_In_PrP_RIV = [str(file) for file in Path(path_RIV).glob("*.idf") if "RIV_Stg" in file.name and MdlN_B_RIV in file.name]
...
rule PrP_RIV:
    ...
    output:
        l_Out_PrP_RIV
    script:
        os.path.join(WS.path_WS, f'code/PrP/edit_RIV/edit_RIV_{MdlN}.py')
