...
Pa_DRN                  =   PJ(Pa_MdlN, 'GWF_1/MODELINPUT/NBR30_SYS1.DRN6')
...
log_MVR_OPTIONS    =   f"{Pa_Smk}/temp/Log_MVR_OPTIONS{MdlN}"
...
rule add_SFR_copy:
    ...
    run:
        ...
        with lock, open(Pa_NAM, 'r+') as f:
            l_lines     = f.readlines()
            l_lines[-1] = f" SFR6 ./GWF_1/MODELINPUT/{MdlN}.SFR6 SFR\n"
            f.seek(0); f.truncate()
            for i in l_lines:
                f.write(i)
...
rule add_MVR_copy: # By copying file. ATM only contains the DRN to SFR items. Later we can add more. 
    input:
        Pa_BAT_RUN,
        Pa_MVR_Src
    output:
        Pa_MVR_Dst
    run:
        sh.copy2(Pa_MVR_Src, Pa_MVR_Dst)
        lock = FL(Pa_NAM + '.lock')  # Create a file lock to prevent concurrent writes
        with lock, open(Pa_NAM, 'r+') as f:
            l_lines     = f.readlines()
            l_lines[-1] = f" MVR6 ./GWF_1/MODELINPUT/{MdlN}.MVR MVR\n"
            f.seek(0); f.truncate()
            for i in l_lines:
                f.write(i)
            f.write('END PACKAGES')

rule add_MVR_to_OPTIONS: # Adds MVR to the OPTIONS file.
    input:
        Pa_BAT_RUN,
        Pa_SFR_Dst
    output:
        log_MVR_OPTIONS
    run:
        U.add_MVR_to_OPTIONS(Pa_SFR_Dst)
        U.add_MVR_to_OPTIONS(Pa_DRN) # Only the 1st system this time.
        pathlib.Path(output[0]).touch()
