...
rule patch_DRN_Sys3: # Patches DRN package to have correct sys3 values.
    input:
        M.Pa.NAM_Sim
    output:
        log_DRN_Sys3_patch
    run:
        ...
        Pa = M.Pa.Sim_In / r'DRN6\SYS3\DRN_T1.ARR'
        DF = pd.read_csv(Pa, names=[col[0] for col in d_Pkg_Cols['DRN']], sep=r'\s+', header=None, skipfooter=12, engine='python')
        DF['cond'] = DF['cond'] * 100
        footer = open(Pa).readlines()[-12:]
        with open(Pa, 'w') as f:
            f.writelines(DF.ws.to_MF_block())
            f.writelines(footer)         
        Path(output[0]).touch() # Create the file to mark the rule as done.
