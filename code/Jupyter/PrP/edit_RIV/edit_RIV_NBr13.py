...
l_RIV_B = [
    f
    for f in LD(Dir_RIV)
    if (MdlN_B in f) and ('RIV_Stg' in f) and ('.IDF' in f.upper()) and ('.dvc' not in f.lower())
]
...
for f in l_RIV_B[:]:
    ...
    IDF = (
        IDF.compute() if IDF.chunks is not None else IDF
    )  # Ensure it's computed if it's a Dask array
