...
with open(f'../../../models/{Mdl}/In/CAP/mete_grid/{Mdl + str(SimN)}/mete_grid.inp', 'w') as f:
    for i, row in DF.iterrows():
        f.write(
            rf'{row["DayOfYear"]:.2f},{row["Year"]},"..\..\In\CAP\P\{Mdl + str(SimN_P)}\P_{row["Date"].strftime("%Y%m%d")}_{Mdl + str(SimN_P)}.asc","..\..\In\CAP\PET\{Mdl + str(SimN_PET)}\PET_{row["Date"].strftime("%Y%m%d")}_{Mdl + str(SimN_PET)}.asc"'
        )
        f.write('\n')
