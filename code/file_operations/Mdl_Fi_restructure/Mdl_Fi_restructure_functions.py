...
def edit_Fi_name(DF, Fi, rename, replace, insert, append):
    ...
    def F_append():
        return DF[Fi].str.split(".").str[0] + DF[append].fillna("") + "." + DF[Fi].str.split(".").str[-1]
...
def replace_in_file_Fi_line(DF, path_Fi_In, path_Fi_Out, C_Src, C_Dst):
    ...
    for src, dst in zip(DF[C_Src], DF[C_Dst]):
        lines = [line.lower().replace(src.lower(), dst) if src.lower() in line.lower()
...
def replace_in_file_Fo_line(DF, path_Fi_In, path_Fi_Out, C_Src, C_Dst, C_replace='replace'):
    ...
    for src, dst, C_replace_val in zip(DF[C_Src], DF[C_Dst], DF[C_replace]):
        ...
        lines = [line.lower().replace(C_replace_val[0].lower(), C_replace_val[1]) 
