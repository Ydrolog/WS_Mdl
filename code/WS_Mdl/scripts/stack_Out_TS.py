...
Pa_clip = (
    Pa_WS
    / r"models\NBr\PoP\common\Pgn\Chaamse_beek\catchment_chaamsebeek_ulvenhout.shp"
)  # Area to clip MSW qrun to; this should be the catchment boundary of the outlet we're analyzing.

l_Sims = ["NBr100", "NBr77", "NBr78"]
...
Pa_Out = (
    Pa_WS / r"models\NBr\PoP\TS\Chaamse Beek Outlet Validation"
)  # Output folder for CSVs and plot.
...
def stack_Out_TS(
    ...
    for i in ["DRN", "RIV", "SFR", "qrun"]:
        try:
            d_DF[i] = pd.read_csv(
                Pa_Out / f"Chaamse_Beek_Outlet-{i}_flows.csv", parse_dates=["date"]
            )
            d_DF[i]["date"] = pd.to_datetime(
                d_DF[i]["date"], format="%d/%m/%Y", errors="coerce"
            )
    ...
    for MdlN in l_Sims[:]:
        ...
        for Pkg in ["DRN", "RIV"]:
            if (
                MdlN not in d_DF[Pkg].columns
            ):  # If this Sim's OBS haven't been appended yet
                try:
                    Pa_OBS_Agg = (
                        M.Pa.MdlN if M.V == "imod5" else M.Pa.MF6
                    ) / f"OBS_Agg/{Pkg}_OBS_Agg_{MdlN}.csv"
                    if Pa_OBS_Agg.exists():
                        DF = pd.read_csv(Pa_OBS_Agg, parse_dates=["date"])[
                            ["date", "SUM"]
                        ]
                        method = "Load"
        ...
        if MdlN not in d_DF["SFR"].columns:
            try:
                ...
                DF["date"] = DT.strptime(str(M.INI.SDATE), "%Y%m%d") + pd.to_timedelta(
                    DF["time"] - 1, unit="D"
                )
        ...
        if MdlN not in d_DF["qrun"].columns:
            try:
                ...
                DA_Qrun_Rio = DA_qrun.rio.set_spatial_dims(
    ...
    # %% Aggregate
    DF_SUM = DF_P.merge(DF_OBS, how="outer", on="date").merge(
        DF_Agg, on="date", how="outer"
    )
    ...
    # %%  Plot
    DF_plot = DF_SUM.loc[
        (DF_SUM["date"] >= date_min) & (DF_SUM["date"] <= date_max)
    ].copy()
    ...
    if not Y_Cols:
        raise ValueError(
            "DF_OBS must contain at least one timeseries column besides 'date'."
        )
    ...
    if not Y_Cols:
        raise ValueError(
            "No numeric timeseries columns available in DF_OBS for plotting."
        )
    ...
    P_Cols = [
        ...
        in {"p", "p (mm/d)", "precipitation", "precipitation (mm/d)"}
    ...
    Pa_Htm = Path(
        Pa_Out
        / f"Outlet_TS_{date_min.strftime('%Y%m%d')}-{date_max.strftime('%Y%m%d')}.html"
    )
